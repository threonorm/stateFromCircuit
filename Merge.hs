...
main =do
	...
	myEdges <-parseFromFile stateGraphStart. (!!1) $ lArgs
	myEdges2 <-parseFromFile stateGraphStart. (!!2) $ lArgs
	case (result,myEdges,myEdges2)  of
		(Right b,Right myE, Right myE2) -> do
				let sg = SG.computeTransitionByCircuit . addIntermediateVariables $b in	
					...
					let graph2 = ((mkGraph (labNodes csg) .fmap 
							(\(x,y) -> (vertexOf csg x, vertexOf csg y,""))
							$ myE2) :: Gr String String) in	
					let compo = last .  sortWith length . DFS.scc $ graph in
					let compo2 = last .  sortWith length . DFS.scc $ graph2 in
					...
					let graphF2 = addLabels 
									((fmap (\x->(x,undefined)) $c_inputs b\\ c_outputs b)
									 ++fc_eqs (addIntermediateVariables b)
									)
						 		$ delNodes (nodes graph2 \\ compo2) graph2 in 
					do
